...
export const ProfileView: React.FC<ProfileViewProps> = ({ onLogout }) => {
  ...
  // Sync initial state once OneSignal is ready (it initializes async)
  useEffect(() => {
    const sync = () => {
    ...
    sync();
    // Poll briefly to catch the async init settling
    const t1 = setTimeout(sync, 600);
    const t2 = setTimeout(sync, 1500);

    // addSubscriptionChangeListener now returns a cleanup fn
    const unsub = addSubscriptionChangeListener((optedIn) => {
  ...
  const handlePurchaseCustomization = async (type: 'title' | 'frame' | 'decoration', value: string, cost: number) => {
    if (studentPoints < cost) {
      alert(`⚠️ Insufficient points. You need ${cost} PTS, but currently have ${studentPoints} PTS.`);
      return;
    }
    if (confirm(`Spend ${cost} points to unlock this enhancement?`)) {
  ...
  const handleToggleNotifications = async () => {
    ...
    // If browser has hard-blocked notifications, guide user to fix it
    if (typeof Notification !== 'undefined' && Notification.permission === 'denied') {
      alert(
        "⚠️ Notifications are blocked in your browser.\n\n" +
        "To fix: Click the 🔒 lock icon in your browser address bar → " +
        "Find 'Notifications' → Change to 'Allow' → Refresh the page."
      );
      return;
    }
    ...
    try {
      ...
      // Wait a moment for OneSignal to settle, then read the real state
      await new Promise(r => setTimeout(r, 800));
      const newState = getPushSubscriptionState();
  ...
  return (
    <div className="relative min-h-screen bg-[#0b1326] text-[#dae2fd] overflow-x-hidden pt-20 pb-28">
      ...
      <div className="fixed bottom-[10%] left-[-10%] w-80 h-80 bg-[#b76dff]/5 rounded-full blur-[100px] -z-20 pointer-events-none" />
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#171f33]/60 backdrop-blur-xl border-b border-white/10 shadow-2xl flex justify-between items-center px-5 py-4 max-w-lg mx-auto">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full border-2 ${profile?.customFrameColor || 'border-[#b76dff]'} flex items-center justify-center bg-[#171f33] overflow-hidden relative`}>
            <span className="font-display font-black text-sm text-[#ddb7ff] tracking-tight">
              {getInitials(studentName)}
            </span>
            {profile?.customAvatarDecoration && (
              <span className="absolute top-0 right-0 text-[10px] animate-pulse">{profile.customAvatarDecoration}</span>
            )}
          </div>
        ...
        >
          <span className="font-mono text-xs font-bold text-[#b76dff] tracking-wider">
      ...
      <main className="px-5 max-w-lg mx-auto space-y-8">
        ...
        <section className="mt-4">
          <div className="relative group perspective">
            ...
            <div className="relative bg-[#171f33]/60 backdrop-blur-xl border border-white/20 rounded-[32px] p-6 shadow-2xl overflow-hidden bg-gradient-to-br from-[#b76dff]/20 to-[#4d8eff]/10">
              ...
              <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-[#b76dff] to-transparent opacity-30 animate-pulse" />
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <h1 className="font-display text-2xl font-extrabold text-[#dae2fd] tracking-tight flex items-center gap-2">
                    {profile?.customAvatarDecoration && <span>{profile.customAvatarDecoration}</span>}
                    <span className={profile?.customFrameColor ? `border-b-2 ${profile.customFrameColor}` : ''}>{studentName}</span>
                  </h1>
                  {profile?.customTitle && (
                    <span className="text-[10px] text-[#ddb7ff] bg-[#b76dff]/25 px-2.5 py-0.5 rounded border border-[#b76dff]/40 font-bold font-mono inline-block mt-2">
                      {profile.customTitle}
                    </span>
                  )}
                  <p className="text-xs text-[#cfc2d6] font-medium mt-1">
                    Class of 2026 • Tech Institute
                  </p>
                </div>
                
                {/* Ranking Badge indicator */}
                <div className="bg-[#b76dff]/10 border border-[#b76dff]/30 rounded-2xl px-3 py-2 backdrop-blur-md shadow-[0_0_15px_rgba(183,109,255,0.3)] text-center">
                  <span className="block font-mono text-[9px] text-[#b76dff] font-bold tracking-wider uppercase leading-none">RANK</span>
                  <span className="block font-display text-xl font-extrabold text-[#ddb7ff] mt-1 leading-none">#42</span>
                </div>
              </div>
                ...
                <div className="space-y-3.5">
                  <div className="flex items-center gap-1.5 select-none">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#b76dff] opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-[#b76dff]"></span>
                    </span>
        ...
        <section className="space-y-3">
          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-[#b76dff]/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                <CreditCard className="w-5 h-5 text-[#b76dff]" />
              </div>
              <span className="text-sm font-semibold text-[#dae2fd]">Payment Methods</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-mono text-[10px] text-[#b76dff] font-bold">2 Linked</span>
              <ChevronRight className="w-5 h-5 text-[#cfc2d6]/70 group-hover:translate-x-0.5 transition-transform" />
            </div>
          </button>

          {/* Menu Item: Notification Settings Toggle */}
          <div className="w-full bg-[#171f33]/60 backdrop-blur-md border border-white/10 flex flex-col gap-2 p-4 rounded-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                  notifBlocked ? 'bg-red-500/10' : 'bg-brand-purple/10'
                }`}>
                  <Bell className={`w-5 h-5 ${
                    notifBlocked
                      ? 'text-red-400'
                      : notificationsEnabled
                        ? 'text-brand-purple-light animate-bounce'
                        : 'text-[#cfc2d6]'
                  }`} />
                </div>
                <div className="flex flex-col">
                  <span className="text-sm font-semibold text-[#dae2fd]">Push Notifications</span>
                  {notifBlocked && (
                    <span className="text-[9px] text-red-400 font-mono mt-0.5">
                      Blocked in browser — tap to fix
                    </span>
                  )}
                </div>
              </div>
              <button
                ...
                className={`px-3 py-1.5 rounded-xl font-mono text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${
                  notifBlocked
                    ? 'bg-red-500/20 text-red-400 border border-red-500/30'
                    : notificationsEnabled
                      ? 'bg-brand-purple text-surface-lowest shadow-md font-bold'
                      : 'bg-[#1e293b] text-zinc-400 hover:text-white border border-white/5'
                }`}
              >
                {notifLoading
                  ? '...'
            ...
            {notificationsEnabled && subscriptionId && (
              <span className="font-mono text-[7px] text-zinc-500 truncate select-all px-1 leading-none mt-1">
                ID: {subscriptionId}
              </span>
            )}
            {!notificationsEnabled && !notifBlocked && (
              <p className="font-sans text-[9px] text-zinc-500 px-1 leading-relaxed mt-0.5">
                Enable to get order-ready alerts, cashier confirmations &amp; wallet updates on your phone — even when the app is closed.
              </p>
            )}
          </div>
        ...
        {/* Profile Customization Shop Section */}
        <section className="space-y-4 bg-[#171f33]/60 backdrop-blur-xl border border-white/10 p-5 rounded-[24px]">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <span className="font-mono text-[9px] font-bold text-brand-purple uppercase tracking-wider block">1. Premium Titles</span>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: "Ramen Master 👑", value: "Ramen Master 👑", cost: 1000 },
                  { label: "Midnight Snacker ⚡", value: "Midnight Snacker ⚡", cost: 1000 },
                  { label: "Chai Emperor ☕", value: "Chai Emperor ☕", cost: 1500 }
                ].map(item => {
                  const isActive = profile?.customTitle === item.value;
                  return (
                    <button
                      key={item.value}
                      onClick={() => handlePurchaseCustomization('title', item.value, item.cost)}
                      className={`p-2.5 rounded-xl border flex flex-col items-center justify-between text-center transition cursor-pointer active:scale-95 min-h-[72px] ${
                        isActive 
                          ? 'border-brand-purple bg-brand-purple/15 text-white' 
                          : 'border-white/5 bg-white/5 text-zinc-400 hover:text-white'
                      }`}
                    >
                      <span className="text-[10px] font-bold leading-tight block">{item.label}</span>
                      <span className="text-[8px] font-mono text-brand-purple-light block mt-1.5">{item.cost} PTS</span>
                    </button>
                  );
                })}
              </div>
            ...
            {/* 2. Custom Border Frames */}
            <div className="space-y-2">
              <span className="font-mono text-[9px] font-bold text-brand-purple uppercase tracking-wider block">2. Neon Frame Borders</span>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: "Purple Laser", value: "border-brand-purple", cost: 800 },
                  { label: "Green Cyber", value: "border-brand-green", cost: 800 },
                  { label: "Golden Crown", value: "border-amber-400", cost: 1200 }
                ].map(item => {
                  const isActive = profile?.customFrameColor === item.value;
                  return (
                    <button
                      key={item.value}
                      onClick={() => handlePurchaseCustomization('frame', item.value, item.cost)}
                      className={`p-2.5 rounded-xl border flex flex-col items-center justify-between text-center transition cursor-pointer active:scale-95 min-h-[72px] ${
                        isActive 
                          ? 'border-brand-purple bg-brand-purple/15 text-white shadow-[0_0_10px_rgba(183,109,255,0.25)]' 
                          : 'border-white/5 bg-white/5 text-zinc-400 hover:text-white'
                      }`}
                    >
                      <span className="text-[10px] font-bold leading-tight block">{item.label}</span>
            ...
            <div className="space-y-2">
              <span className="font-mono text-[9px] font-bold text-brand-purple uppercase tracking-wider block">3. Avatar Decorations</span>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: "Stars ✨", value: "✨", cost: 500 },
                  { label: "Fire 🔥", value: "🔥", cost: 500 },
                  { label: "Crown 👑", value: "👑", cost: 500 }
                ].map(item => {
                  const isActive = profile?.customAvatarDecoration === item.value;
                  return (
                    <button
                      key={item.value}
                      onClick={() => handlePurchaseCustomization('decoration', item.value, item.cost)}
                      className={`p-2.5 rounded-xl border flex flex-col items-center justify-between text-center transition cursor-pointer active:scale-95 min-h-[72px] ${
                        isActive 
                          ? 'border-brand-purple bg-brand-purple/15 text-white' 
                          : 'border-white/5 bg-white/5 text-zinc-400 hover:text-white'
                    ...
                    >
                      <span className="text-xl block">{item.value}</span>
                      <span className="text-[8px] font-mono text-brand-purple-light block mt-1.5">{item.cost} PTS</span>
                    </button>
                  );
                })}
          ...
          <div className="bg-[#171f33]/60 backdrop-blur-xl border border-[#b76dff]/20 p-5 rounded-[24px]">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button 
                ...
                className={`flex flex-col items-center gap-3 p-3 rounded-2xl border-2 transition-all cursor-pointer ${
                  activeTheme === 'LIGHT'
                    ? 'bg-brand-purple/20 border-brand-purple shadow-[0_0_15px_rgba(183,109,255,0.35)]'
                    : 'bg-white/5 border-transparent opacity-60 hover:opacity-100'
                }`}
              >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2.5 font-sans">
                  <div className="flex gap-2 text-xs text-zinc-400">
                    <CheckCircle2 className="w-4 h-4 text-[#b76dff] shrink-0" />
                    <span><strong>Skip-The-Line:</strong> Priority checkout at all central central server lanes.</span>
                  </div>
                  <div className="flex gap-2 text-xs text-zinc-400">
                    <CheckCircle2 className="w-4 h-4 text-[#b76dff] shrink-0" />
